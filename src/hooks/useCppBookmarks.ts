...
export function useCppBookmarks(){
  ...
  useEffect(()=>{
    ...
    const load = async ()=>{
      try{
        const { data, error } = await supabase.from("core_java_user_state").select("question_id, is_bookmarked").eq("user_id", user.id).eq("is_bookmarked", true).like("question_id","cpp:%");
        if(error){ console.warn("Could not load cpp bookmarks:", error.message); setLoading(false); return; }
