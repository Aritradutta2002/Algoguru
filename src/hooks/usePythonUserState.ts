...
export interface PythonUserState {
...
export function usePythonUserState(): PythonUserState {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try{
        const { data, error } = await supabase.from("core_java_user_state").select("question_id, notes, is_completed, updated_at, reading_section").eq("user_id", user.id).like("question_id","py:%");
        if(error){ console.warn("Could not load python user state:", error.message); setLoading(false); return; }
