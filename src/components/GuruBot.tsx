...
function ModelSelector({ selected, onSelect, isMobile }: { selected: string; onSelect: (k: string) => void; isMobile?: boolean }) {
  ...
  useEffect(() => {
    const handleClick = (e: MouseEvent) => {
      if (open && dropRef.current && !dropRef.current.contains(e.target as Node) && !triggerRef.current?.contains(e.target as Node)) {
        setOpen(false);
      }
    };
        ...
        >
          ...
          <div className="relative z-10 p-2 space-y-1">
            {MODELS.map((m) => (
              ...
              >
                <div className={`w-1.5 h-1.5 rounded-full transition-all duration-500 ${m.key === selected ? "bg-primary scale-125 shadow-[0_0_12px_hsl(var(--primary))]" : "bg-muted-foreground/20"}`} />
                <div className="flex-1 min-w-0">
                  <span className={`text-[12px] font-bold tracking-tight transition-colors ${m.key === selected ? "text-foreground" : "text-muted-foreground group-hover:text-foreground"}`}>{m.label}</span>
                </div>
                <span className={`text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded-lg border transition-colors ${
                  m.key === selected ? "bg-primary/20 border-primary/30 text-primary" : "bg-muted/50 border-border/20 text-muted-foreground/40 group-hover:text-muted-foreground/60"
                }`}>
                  {m.tag}
                </span>
              </button>
            ...
            <div className="p-6">
              <div className="flex items-center justify-between mb-8 px-1">
                <div className="space-y-1">
                  <h3 className="text-[10px] font-black uppercase tracking-[0.25em] text-muted-foreground/30">Intelligence History</h3>
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-black uppercase tracking-tight text-foreground/80">Recent Chats</span>
                    <span className="text-[9px] font-black px-2 py-0.5 rounded-full bg-muted border border-border/20 text-muted-foreground/40">{sessions.length}</span>
                  </div>
                </div>
              </div>
              <div className="space-y-2">
                {sessions.length === 0 ? (
                  <div className="text-center py-20">
                    ...
                    <p className="text-muted-foreground/40 text-[11px] font-black uppercase tracking-widest">Your archive is empty</p>
                  </div>
                ) : (
                  sessions.map(s => (
                    <div 
                    ...
                    >
                      <div className="flex items-center gap-4 overflow-hidden">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-300 ${
                          s.id === currentId ? 'bg-primary text-primary-foreground shadow-lg shadow-primary/20' : 'bg-muted/50 group-hover:bg-card border border-border/10'
                        }`}>
                          <MessageSquare size={14} />
                        </div>
                        <div className="truncate text-[13px] font-bold tracking-tight">{s.title}</div>
                      </div>
                      <button 
          ...
          <div className="h-full overflow-y-auto p-4 md:p-6 lg:p-8 space-y-8" style={{ overscrollBehavior: 'contain' }}>
            ...
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-3`}>
                <div className={`flex gap-5 max-w-[94%] ${m.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                  {m.role === 'assistant' && (
                            ...
                            code({ className, children, ...props }) {
                              ...
                              return (
                                <code
                                  ...
                                  style={{ background: 'hsl(var(--primary) / 0.05)', color: 'hsl(var(--primary))' }}
                                  {...props}
                                >{children}</code>
                              );
                            },
            ...
            {loading && messages[messages.length - 1]?.role !== 'assistant' && (
              <div className="flex gap-5 items-start animate-in fade-in">
                ...
                <div className="flex gap-2 pt-5 pl-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-primary animate-bounce shadow-[0_0_8px_hsl(var(--primary))]" style={{ animationDelay: '0ms' }} />
                  <span className="w-1.5 h-1.5 rounded-full bg-primary animate-bounce shadow-[0_0_8px_hsl(var(--primary))]" style={{ animationDelay: '200ms' }} />
                  <span className="w-1.5 h-1.5 rounded-full bg-primary animate-bounce shadow-[0_0_8px_hsl(var(--primary))]" style={{ animationDelay: '400ms' }} />
                </div>
              </div>
            )}
