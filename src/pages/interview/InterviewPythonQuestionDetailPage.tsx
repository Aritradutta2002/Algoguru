...
import { CppQuestionAnswer } from "@/components/interview/CppQuestionAnswer";
import {
  CppBookmarkButton,
  CppLearnedButton,
  CppShareButton,
  CppCopyTextButton,
} from "@/components/interview/CppActions";
import { CppDifficultyBadge, CppPriorityBadge } from "@/components/interview/CppBadges";
...
import {
  ...
  getAdjacentPythonQuestions,
  getPythonQuestionById,
} from "@/lib/pythonQuestionIndex";
import { getPythonQuestionDetailPath, getPythonQuestionMeta } from "@/data/pythonInterviewMetadata";
...
function buildTocSections(questionId: string): TocSection[] {
  ...
  const question = getPythonQuestionById(questionId);
  ...
  const meta = getPythonQuestionMeta(questionId);
...
export default function InterviewPythonQuestionDetailPage() {
  const { questionSlug, language } = useParams<{ questionSlug?: string; language?: string }>();
  const navigate = useNavigate();
  const { toast } = useToast();
  const { user, loading: authLoading } = useAuth();
  const { isBookmarked, toggleBookmark } = usePythonBookmarks();
  const {
    doneMap,
    toggleDone,
    isUpserting,
  ...
  const entry = questionSlug ? getPythonQuestionBySlug(questionSlug) : undefined;
  ...
  // Restore reading position once when the page loads.
  ...
  useEffect(() => {
    if (question) {
      document.title = `${question.question} — Python Interview | AlgoGuru`;
    }
    return () => {
  ...
  if (!entry || !question) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-background text-foreground px-4">
        ...
        <button
          onClick={() => navigate(`/interview/${language ?? "python"}/language-questions`)}
  ...
  const { previous, next } = getAdjacentPythonQuestions(question.id);
  const listPath = `/interview/${language ?? "python"}/language-questions`;
  const shareUrl = typeof window !== "undefined" ? window.location.href : getPythonQuestionDetailPath(question);
  const quickAnswer = answerSections[0] ?? "";
  const hasDeepDive = answerSections.length > 1;

  const handleQuestionNavigate = (slug: string) => {
    skipRestoreRef.current = true;
    navigate(`/interview/${language ?? "python"}/language-questions/${slug}`);
    ...
    <div className="cjd-page min-h-screen bg-background text-foreground">
      ...
      <header className="cjd-sticky-bar sticky top-0 z-30 border-b bg-card/95 backdrop-blur-md">
        <div className="max-w-[1440px] mx-auto px-4 md:px-8 flex items-center gap-3 py-2.5">
          ...
          <CppBookmarkButton questionId={question.id} isBookmarked={isBookmarked(question.id)} onToggle={handleBookmark} compact />
          <CppLearnedButton questionId={question.id} isLearned={!!doneMap[question.id]} isUpserting={isUpserting(question.id)} onToggle={handleLearned} compact />
          <CppShareButton url={shareUrl} title={question.question} compact />
        </div>
      </header>
      ...
      <div className="max-w-[1440px] mx-auto px-4 md:px-8 py-6 md:py-10 cjd-grid">
        ...
        <main className="cjd-main min-w-0">
          ...
          <nav aria-label="Breadcrumb" className="flex items-center gap-1.5 text-[11px] text-muted-foreground mb-6 flex-wrap font-mono">
            ...
            <Link to="/interview/python" className="hover:text-primary transition-colors">Interview</Link>
            <ChevronRight size={11} aria-hidden="true" />
            <Link to={listPath} className="hover:text-primary transition-colors">Python</Link>
            <ChevronRight size={11} aria-hidden="true" />
          ...
          <article className="cjd-article" aria-labelledby="question-title">
            ...
            <header className="cjd-question-header mb-8">
              <div className="flex items-center gap-2.5 flex-wrap mb-4">
                ...
                {meta.difficulty && <CppDifficultyBadge difficulty={meta.difficulty} />}
                {meta.priority && <CppPriorityBadge priority={meta.priority} />}
                {meta.pythonVersions?.map((v) => (
                  <span key={v} className="inline-flex items-center px-2.5 py-1 rounded-full border text-[10px] font-bold uppercase tracking-wider font-mono bg-info/10 text-info border-info/25">
                    {v}
                  </span>
                ))}
            ...
            <section id="quick-answer" className="cjd-section scroll-mt-36" aria-labelledby="quick-answer-heading">
              ...
              <div className="cjd-surface rounded-xl p-5 md:p-6">
                <CppQuestionAnswer answer={quickAnswer} />
              </div>
            </section>
            ...
            {hasDeepDive && (
              <section id="detailed-answer" className="cjd-section scroll-mt-36" aria-labelledby="detailed-answer-heading">
                ...
                <div className="cjd-surface rounded-xl p-5 md:p-6">
                  <CppQuestionAnswer answer={answerSections.slice(1).join("\n\n")} />
                </div>
              </section>
            )}
            ...
            {question.code && (
              <section id="code-example" className="cjd-section scroll-mt-36" aria-labelledby="code-example-heading">
                ...
                <CodeBlock language={question.codeLanguage || "python"} code={question.code} title="Example" />
              </section>
            )}
              ...
              <section id="related-questions" className="cjd-section scroll-mt-36" aria-labelledby="related-questions-heading">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {meta.relatedQuestionIds.map((rid) => {
                    const related = getPythonQuestionById(rid);
                    if (!related) return null;
                    return (
                      <Link
                        key={rid}
                        to={getPythonQuestionDetailPath(related.question)}
            ...
            <div className="flex items-center gap-2.5 flex-wrap mt-10 pt-6 border-t border-border/30">
              <CppBookmarkButton questionId={question.id} isBookmarked={isBookmarked(question.id)} onToggle={handleBookmark} />
              <CppLearnedButton questionId={question.id} isLearned={!!doneMap[question.id]} isUpserting={isUpserting(question.id)} onToggle={handleLearned} />
              <CppShareButton url={shareUrl} title={question.question} />
              {question.explanation && (
                <CppCopyTextButton text={`${question.question}\n\n${question.explanation}`} label="Copy Summary" />
              )}
            </div>
