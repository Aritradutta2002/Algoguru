...
import { CppBookmarkButton } from "@/components/interview/CppActions";
...
import { getAllPythonQuestions, type IndexedPythonQuestion } from "@/lib/pythonQuestionIndex";
...
const _QuestionCard = ({
  ...
  const detailPath = getPythonQuestionDetailPath(question);
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mb-2 flex-wrap">
              ...
              {meta.priority === "very-high" && <CppPriorityBadge priority="very-high" />}
            ...
            <div className="p-5 md:p-6 space-y-4">
              {activeView === "theory" && (
                <div className="bg-muted/30 rounded-xl p-5 md:p-6 border border-border/20 shadow-inner">
                  <CppQuestionAnswer answer={question.answer} />
                </div>
              )}
...
// Page
// ─────────────────────────────────────────────────────────────────────────
export default function InterviewPythonQuestionsPage() {
  const { language } = useParams<{ language?: string }>();
  const navigate = useNavigate();
  const [searchParams, setSearchParams] = useSearchParams();
  const { toast } = useToast();
  const { user, loading: authLoading } = useAuth();
  const { doneMap, notesMap, upsertingId, toggleDone, saveNote, deleteNote } = usePythonUserState();
  const { isBookmarked, toggleBookmark, bookmarkedIds } = usePythonBookmarks();
  ...
  // SEO title
  useEffect(() => {
    document.title = "Python Interview Questions | AlgoGuru";
    return () => {
  ...
  const groupedTopics = useMemo(() => {
    const order = new Map(pythonInterviewTopics.map((t) => [t.id, t]));
    const groups = new Map<string, { topic: (typeof pythonInterviewTopics)[number]; entries: IndexedPythonQuestion[] }>();
  ...
  const downloadNotesPDF = () => {
    ...
    let y = 20;
    doc.setFontSize(22);
    doc.text("Python Interview Notes", 20, y);
    y += 15;
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <h1 className="text-lg md:text-xl font-bold tracking-tight truncate">
                  Python <span className="text-primary">Q&A</span>
                </h1>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 md:gap-3 flex-wrap justify-end shrink-0">
            ...
            <div className="relative hidden md:block">
              ...
              <input
                ...
                aria-label="Search Python interview questions"
                  ...
                  return (
                    ...
                    >
                      <div className="pr-8 mb-3">
                        {questionData && (
                          <Link
                            to={getPythonQuestionDetailPath(questionData.question)}
