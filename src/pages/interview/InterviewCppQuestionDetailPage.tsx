...
import { CppVisualizationBlock, hasCppVisualization } from "@/components/interview/CppVisualizationBlock";
import {
  CppBookmarkButton,
  CppLearnedButton,
  CppShareButton,
...
import { CppDifficultyBadge, CppPriorityBadge, CppVersionBadge } from "@/components/interview/CppBadges";
...
import {
  getCppQuestionBySlug,
  getAdjacentCppQuestions,
  getCppQuestionById,
} from "@/lib/cppQuestionIndex";
import { getCppQuestionDetailPath, getCppQuestionMeta } from "@/data/cppInterviewMetadata";
...
function buildTocSections(questionId: string): TocSection[] {
  ...
  const meta = getCppQuestionMeta(questionId);
...
export default function InterviewCppQuestionDetailPage() {
  const { questionSlug, language } = useParams<{ questionSlug?: string; language?: string }>();
  const navigate = useNavigate();
  const { toast } = useToast();
  const { user, loading: authLoading } = useAuth();
  const { isBookmarked, toggleBookmark } = useCppBookmarks();
  const {
    doneMap,
    toggleDone,
    isUpserting,
  ...
  const entry = questionSlug ? getCppQuestionBySlug(questionSlug) : undefined;
  ...
  useEffect(() => {
    if (question) {
      document.title = `${question.question} — C++ Interview | AlgoGuru`;
    }
    return () => {
  ...
  if (!entry || !question) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-background text-foreground px-4">
        ...
        <button
          onClick={() => navigate(`/interview/${language ?? "cpp"}/language-questions`)}
  ...
  const { previous, next } = getAdjacentCppQuestions(question.id);
  const listPath = `/interview/${language ?? "cpp"}/language-questions`;
  const shareUrl = typeof window !== "undefined" ? window.location.href : getCppQuestionDetailPath(question);
  const quickAnswer = answerSections[0] ?? "";
  const hasDeepDive = answerSections.length > 1;

  const handleQuestionNavigate = (slug: string) => {
    skipRestoreRef.current = true;
    navigate(`/interview/${language ?? "cpp"}/language-questions/${slug}`);
      ...
      <div className="max-w-[1440px] mx-auto px-4 md:px-8 py-6 md:py-10 cjd-grid">
        ...
        <main className="cjd-main min-w-0">
          ...
          <nav aria-label="Breadcrumb" className="flex items-center gap-1.5 text-[11px] text-muted-foreground mb-6 flex-wrap font-mono">
            ...
            <Link to="/interview/cpp" className="hover:text-primary transition-colors">Interview</Link>
            <ChevronRight size={11} aria-hidden="true" />
            <Link to={listPath} className="hover:text-primary transition-colors">C++</Link>
            <ChevronRight size={11} aria-hidden="true" />
          ...
          <article className="cjd-article" aria-labelledby="question-title">
            ...
            <header className="cjd-question-header mb-8">
              <div className="flex items-center gap-2.5 flex-wrap mb-4">
                ...
                {meta.cppVersions?.map((v) => <CppVersionBadge key={v} version={v} />)}
            ...
            {/* ── Visualization ───────────────────────────────────── */}
            {hasCppVisualization(question.id) && (
              <div id="visualization" className="scroll-mt-36">
                <CppVisualizationBlock questionId={question.id} />
              </div>
            )}
            ...
            {question.code && (
              <section id="code-example" className="cjd-section scroll-mt-36" aria-labelledby="code-example-heading">
                ...
                <CodeBlock language={question.codeLanguage || "cpp"} code={question.code} title="Example" />
              </section>
            )}
              ...
              <section id="related-questions" className="cjd-section scroll-mt-36" aria-labelledby="related-questions-heading">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {meta.relatedQuestionIds.map((rid) => {
                    const related = getCppQuestionById(rid);
                    if (!related) return null;
                    return (
                      <Link
                        key={rid}
                        to={getCppQuestionDetailPath(related.question)}
