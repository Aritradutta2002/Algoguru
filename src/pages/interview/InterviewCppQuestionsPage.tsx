...
import { getAllCppQuestions, type IndexedCppQuestion } from "@/lib/cppQuestionIndex";
import { getCppQuestionDetailPath } from "@/data/cppInterviewMetadata";
import { hasCppVisualization } from "@/components/interview/CppVisualizationBlock";
...
const _QuestionCard = ({
  ...
  const detailPath = getCppQuestionDetailPath(question);
...
// Page
// ─────────────────────────────────────────────────────────────────────────
export default function InterviewCppQuestionsPage() {
  const { language } = useParams<{ language?: string }>();
  const navigate = useNavigate();
  const [searchParams, setSearchParams] = useSearchParams();
  const { toast } = useToast();
  const { user, loading: authLoading } = useAuth();
  const { doneMap, notesMap, upsertingId, toggleDone, saveNote, deleteNote } = useCppUserState();
  const { isBookmarked, toggleBookmark, bookmarkedIds } = useCppBookmarks();
  ...
  // SEO title
  useEffect(() => {
    document.title = "C++ Interview Questions | AlgoGuru";
    return () => {
  ...
  const groupedTopics = useMemo(() => {
    const order = new Map(cppInterviewTopics.map((t) => [t.id, t]));
    const groups = new Map<string, { topic: (typeof cppInterviewTopics)[number]; entries: IndexedCppQuestion[] }>();
  ...
  const downloadNotesPDF = () => {
    ...
    let y = 20;
    doc.setFontSize(22);
    doc.text("C++ Interview Notes", 20, y);
    y += 15;
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <h1 className="text-lg md:text-xl font-bold tracking-tight truncate">
                  C++ <span className="text-primary">Q&A</span>
                </h1>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 md:gap-3 flex-wrap justify-end shrink-0">
            ...
            <div className="relative hidden md:block">
              ...
              <input
                ...
                aria-label="Search C++ interview questions"
                  ...
                  return (
                    ...
                    >
                      <div className="pr-8 mb-3">
                        {questionData && (
                          <Link
                            to={getCppQuestionDetailPath(questionData.question)}
