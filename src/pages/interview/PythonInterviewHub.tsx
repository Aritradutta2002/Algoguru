...
import { ArrowRight, Bookmark, CheckCircle2, BrainCircuit, Flame, Layers, Sparkles, Target, Timer, TrendingUp } from "lucide-react";
import { pythonInterviewTopics } from "@/data/pythonInterviewData";
import { usePythonUserState } from "@/hooks/usePythonUserState";
import { usePythonBookmarks } from "@/hooks/usePythonBookmarks";
import { getAllPythonQuestions, getPythonTopicCount } from "@/lib/pythonQuestionIndex";
import { getPythonQuestionDetailPath } from "@/data/pythonInterviewMetadata";
...
const LEARNING_PATHS = [
  { id: "python-qa", title: "Python Q&A", subtitle: "Theory, code & interview-ready answers", icon: BrainCircuit, color: "primary" as const, route: "/interview/python/language-questions" },
  { id: "data-structure", title: "Data Structure", subtitle: "DSA patterns used in Python rounds", icon: Target, color: "accent" as const, route: "/interview/python/data-structure" },
  { id: "system-design", title: "System Design", subtitle: "Scalable system thinking", icon: Layers, color: "destructive" as const, route: "/interview/python/system-design" },
  { id: "sql-structure", title: "SQL Questions", subtitle: "Interview SQL concepts & patterns", icon: Flame, color: "info" as const, route: "/interview/python/sql-structure" },
];
...
function countDifficulty(questions: ReturnType<typeof getAllPythonQuestions>, difficulty: string){ return questions.filter((q)=> q.meta.difficulty===difficulty).length; }

export default function PythonInterviewHub(){
  const navigate = useNavigate();
  const { doneMap } = usePythonUserState();
  const { bookmarkedIds } = usePythonBookmarks();
  const allQuestions = useMemo(()=> getAllPythonQuestions(), []);
  const totalQuestions = allQuestions.length;
  const topicCount = getPythonTopicCount();
  ...
  const topicStats = useMemo(()=> pythonInterviewTopics.map((topic,i)=>{ const done=topic.questions.filter((q)=> doneMap[q.id]).length; return { topic, number:i+1, done, total: topic.questions.length, pct: topic.questions.length>0?Math.round((done/topic.questions.length)*100):0 }; }),[doneMap]);
  ...
  const learningPaths = LEARNING_PATHS.map((p)=> p.id==="python-qa"?{...p, subtitle:`${totalQuestions} interview questions with answers`}:p);
      ...
      <div className="w-full max-w-[1800px] mx-auto px-4 md:px-10 lg:px-16 py-8 md:py-12">
        ...
        <motion.header initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.55, ease:[0.22,1,0.36,1]}} className="cjh-hero p-6 md:p-10 lg:p-12 mb-10">
          ...
          <div className="relative z-10 grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
            <div>
              <div className="cjh-hero-badge mb-5"><BrainCircuit size={13}/> Python Interview Preparation</div>
              <h1 className="cjh-hero-title text-3xl md:text-[2.75rem] lg:text-5xl 2xl:text-[3.5rem] font-bold leading-[1.1] mb-4">
                Master Python. Prepare smarter. <span>Crack the interview.</span>
              </h1>
              <p className="text-[15px] md:text-base text-muted-foreground leading-relaxed mb-7 max-w-xl">
                Structured path through Python basics, strings & collections, functions & generators, OOP & decorators, I/O & memory, and concurrency — with code snippets and interview-ready answers.
              </p>
              <div className="flex flex-wrap items-center gap-3">
                <button type="button" onClick={()=> continueQuestion ? navigate(getPythonQuestionDetailPath(continueQuestion.question)) : navigate("/interview/python/language-questions")} className="cjh-btn-primary">
                  {progressPct>0?"Continue Learning":"Start Learning"} <ArrowRight size={15}/>
                </button>
                <Link to="/interview/python/language-questions" className="cjh-btn-secondary">Explore Questions</Link>
              </div>
            </div>
            <dl className="grid grid-cols-2 gap-3">
              {[{label:"Questions", value:`${totalQuestions}+`},{label:"Topics", value:String(topicCount)},{label:"Difficulty Mix", value:null, custom:(<span className="text-xs font-bold leading-relaxed"><span className="text-success">{easyCount}E</span>{" · "}<span className="text-warning">{mediumCount}M</span>{" · "}<span className="text-destructive">{hardCount}H</span></span>)},{label:"Python Versions", value:"3.8 – 3.13"}].map((stat)=>(
        ...
        {progressPct>0 ? (
          <motion.section initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} transition={{duration:0.45, delay:0.1}} className="cjh-progress-card p-5 md:p-6 mb-12" aria-label="Your Python interview progress">
            ...
            {continueQuestion && <button type="button" onClick={()=> navigate(getPythonQuestionDetailPath(continueQuestion.question))} className="mt-4 cjh-btn-primary text-xs py-2 px-4 min-h-0">Pick up where you left off <ArrowRight size={13}/></button>}
          </motion.section>
        ) : (
          <motion.button type="button" initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} transition={{duration:0.45, delay:0.1}} onClick={()=> continueQuestion ? navigate(getPythonQuestionDetailPath(continueQuestion.question)) : navigate("/interview/python/language-questions")} className="cjh-progress-card w-full px-5 py-4 mb-12 flex items-center justify-between gap-4 group text-left" aria-label="Start your Python interview progress">
          ...
          <div className="cjh-roadmap grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
            {topicStats.map(({topic,number,done,total,pct},i)=>(
              <motion.div key={topic.id} variants={fadeUp} transition={{delay:i*0.03}}>
                <Link to={`/interview/python/language-questions?topic=${topic.id}`} className={`cjh-topic-card group ${pct===100?"cjh-topic-card--complete":""}`}>
        ...
        <motion.section className="mb-14" aria-labelledby="most-asked-heading" variants={stagger} initial="initial" whileInView="animate" viewport={{once:true, margin:"-40px"}}>
          <div className="cjh-section-header">
            ...
            <Link to="/interview/python/language-questions?filter=most-asked" className="text-xs font-semibold text-primary hover:underline inline-flex items-center gap-1">View all <ArrowRight size={12}/></Link>
          </div>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-6">
            {mostAsked.map((entry,i)=>(
              <motion.div key={entry.question.id} variants={fadeUp} transition={{delay:i*0.05}}>
                <Link to={getPythonQuestionDetailPath(entry.question)} className="cjh-question-card group">
                  <div className="flex items-center gap-2 mb-2.5 flex-wrap">
                    ...
                    {entry.meta.difficulty && <CppDifficultyBadge difficulty={entry.meta.difficulty as any}/>}
                  </div>
                  ...
                  <div className="mt-auto">{entry.meta.priority && <CppPriorityBadge priority={entry.meta.priority as any}/>}</div>
                </Link>
              </motion.div>
            ))}
          </div>
        ...
        <motion.section className="mb-14" aria-labelledby="quick-revision-heading" initial={{opacity:0,y:16}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:0.5}}>
          <div className="cjh-revision-banner p-6 md:p-8 flex flex-col md:flex-row md:items-center gap-6 relative z-10">
            ...
            <button type="button" onClick={()=> navigate("/interview/python/language-questions?filter=most-asked")} className="cjh-btn-primary shrink-0">Start Quick Revision <ArrowRight size={15}/></button>
          </div>
        </motion.section>
